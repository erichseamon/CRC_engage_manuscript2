...
ggplot(crcdata_new, aes(fill=ghodsvali, x=year)) + 
    geom_bar(position="stack", stat="count")+theme_bw()+theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + scale_x_continuous(breaks = crcdata_new$year) + scale_fill_manual(values = my_colors)
...
stratio_vs_solutions <-glm(solution_proposed_YN ~ ST_farmers + ST_combined_gov + ST_tribal + ST_combined_coalition + ST_combined_industry + ST_migrants + ST_youth + ST_public + ST_university + ST_experts,family=binomial,data=crcdata)
